...
/**
 * A3Objectをフルスクリーンで表示するためのクラスです。
 * 基本的に使用方法はA3WindowやJA3Windowなどと同じなので，
 * サンプルプログラムや詳細はそちらを参照して下さい．
 * フルスクリーンの場合ウィンドウと違って「ウィンドウを閉じる」
 * ボタンが無いので，かわりに「Ctrl+Alt+C」で強制終了するように
 * しています．
 */
public class A3Screen implements A3CanvasInterface {
    ...
    /**
     * A3Objectをフルスクリーンで表示するためのクラスを生成します。
     */
    public A3Screen() {
        ...
        try {
            EventQueue.invokeAndWait(new Runnable() {
                public void run() {
                    ...
                    canvas = A3Canvas.createA3Canvas();
                    frame = new Frame();
                    frame.add(canvas);
